...
function findCaliforniaCafes(str) {
    ...
    for (let i = 0; i < cafes.length; i++) {
        let placeObj;
        if (cafes[i].name.includes(str)) {
            placeObj = newPlacesObj[cafes[i].place_id];
            let newOtherObj = {};
            newOtherObj.name = cafes[i].name;
            newOtherObj.street_no = placeObj.street_no;
            newOtherObj.locality = placeObj.locality;
            newOtherObj.postal_code = placeObj.postal_code;
            newOtherObj.lat = placeObj.lat;
            newOtherObj.long = placeObj.long;

            finalArray.push(newOtherObj);
        }
    }
    console.log(finalArray);
}

findCaliforniaCafes("Avenue")
